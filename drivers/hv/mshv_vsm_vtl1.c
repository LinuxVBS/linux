...
static int hv_modify_vtl_protection_mask(u64 start_pfn, u64 number_of_pages, u32 page_access)
{
	...
	pr_debug("%s start = 0x%llx, page_count = %lld, perm = 0x%x\n",
		 __func__, start_pfn, number_of_pages, page_access);
	...
	while (total_pages_processed < number_of_pages) {
		for (i = 0; ((i < max_pages_per_request) &&
			...
			hvin->gpa_page_list[i] = start_pfn + total_pages_processed + i;
...
static int __init mshv_vtl1_init(void)
{
	...
	if (hv_vsm_init_code_page_offsets()) {
	...
	for (i = 0; i < E820_MAX_ENTRIES_ZEROPAGE; i++) {
		if (e820_table[i].type == E820_TYPE_RAM) {
			...
			pr_debug("VSM: Protect VTL1 memory region 0x%llx:0x%llx:%lld\n",
				 start, end, page_count);
			ret = hv_modify_vtl_protection_mask(start >> PAGE_SHIFT, page_count,
							    permissions);
			if (ret)
